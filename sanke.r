...
stage1<-function(){
  ...
  bla<-function(){
    ...
    if(e$bla[2] == 1 | e$bla[2] == 20) {
      e$bla_B=e$bla_B * -1  # 朝着反方向，定义障碍物在画板中间移动
    }
  }
  # 失败检查
  fail<-function(){
  ...
  # head():生成蛇头移动坐标
  head<-function(){      # 生成蛇头坐标
    e$lastx<-e$head[1]
    e$lasty<-e$head[2]
    
    
    if(e$dir=='up') e$head[2]<-e$head[2]+1    # 方向操作，向上移动
    if(e$dir=='down') e$head[2]<-e$head[2]-1  # 方向操作，向下移动
    if(e$dir=='left') e$head[1]<-e$head[1]-1  # 方向操作，向左移动
    if(e$dir=='right') e$head[1]<-e$head[1]+1 # 方向操作，向右移动
  ...
  body<-function(){         
    ...
    if(e$eat_num == 3){# e$eat_num表示连吃果子数，如果连吃果子数等于3
...
# 键盘事件
keydown<-function(K){
  print(paste("keydown:",K,",stage:",e$stage));  #打印键盘输入
...
run<-function(){
  par(mai=rep(0,4),oma=rep(0,4))  #设置全局画布
  e<<-new.env()  #定义全局环境变量，用于封装变量
  stage0()  #启动开机场景
